...
export async function getTrajet(id_bus: string,M_S:number) {
  ...
  export async function getTrajetParDate(date:string,id_bus: string,M_S:number) {
    try {
        console.log(id_bus)
      const response = await axios.get(`${process.env.REACT_APP_API_BFF_ADMIN_URL}/trajet/busTrajet/date`,{
        params: {
          date:date,
          id_bus: id_bus,
          M_S: M_S
